...
CREATE TABLE IF NOT EXISTS qris_payment_attempts (
    id INT AUTO_INCREMENT PRIMARY KEY,
    user_id INT NOT NULL,
    target_id INT NOT NULL,
    amount DECIMAL(15, 2) NOT NULL,
    panzngen_keyorkut VARCHAR(255) NULL UNIQUE, -- Kunci/ID transaksi dari PanzNGen (jika ada dan unik)
    panzngen_payment_data TEXT NULL,           -- Untuk menyimpan data QR atau info lain dari PanzNGen
    status ENUM('PENDING', 'SUCCESS', 'FAILED', 'EXPIRED', 'CHECKING') DEFAULT 'PENDING',
    our_transaction_ref VARCHAR(255) UNIQUE,   -- ID unik internal kita untuk percobaan pembayaran ini
...
-- Indeks untuk referensi internal kita
CREATE INDEX idx_our_transaction_ref ON qris_payment_attempts(our_transaction_ref);
